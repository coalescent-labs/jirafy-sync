...
describe('Jirafy Sync', () => {
  ...
  context('Jira Projects', () => {
    it('Ensure jira project keys are always uppercase', async function () {
      if (!projectId) this.skip()
      var resp = await jira.getProjectIdByKey(key.toLowerCase())
      chai.assert.equal(resp, projectId)
    })

    it('Ensure project retrieval error message', async function () {
      if (!projectId) this.skip()
    ...
    it('Ensure project retrieval success', async function () {
      if (!projectId) this.skip()
